...
import { parseRegalJson, parseRegalJsonWithStats, parseRegalDatePayload } from "@/scraper/parseRegal";
...
describe("parseRegalJson", () => {
  ...
  describe("theatre filtering (FIX 3)", () => {
    const makePayload = (theatreCode: string | number, performanceId: number) => ({
      showDate: "2026-07-28T00:00:00",
      shows: [
        {
    ...
    it("matches TheatreCode '0347' against expectedTheatreCode '347' (no leading zero)", () => {
      const payload = makePayload("0347", 1303);
      const result = parseRegalJson([payload], "347");
      expect(result).toHaveLength(1);
    });

    it("matches numeric TheatreCode 347 against expectedTheatreCode '0347'", () => {
      const payload = makePayload(347, 1304);
      const result = parseRegalJson([payload], "0347");
      expect(result).toHaveLength(1);
    });

    it("matches TheatreCode '347' (no leading zero) against expectedTheatreCode '0347'", () => {
      const payload = makePayload("347", 1305);
      const result = parseRegalJson([payload], "0347");
      expect(result).toHaveLength(1);
    });

    it("still excludes a genuinely different theatre and counts it in theatreMismatch", () => {
      const payload = makePayload("9999", 1306);
      const { showtimes, stats } = parseRegalJsonWithStats([payload], "0347");
      expect(showtimes).toHaveLength(0);
      expect(stats.theatreMismatch).toBe(1);
    });
  });

  describe("theatre-scoped dedup (cross-date PerformanceId collision)", () => {
    it("dedupes a PerformanceId that reappears on a later date when the same seen map is shared", () => {
      const makeDatePayload = (dateIso: string, performanceId: number, utcShowTime: string) => ({
        showDate: dateIso,
        shows: [
          {
            TheatreCode: "1484",
            AdvertiseShowDate: dateIso,
            Film: [
              {
                Title: "Film Z",
                MasterMovieCode: "FILM_Z",
                Performances: [
                  {
                    PerformanceId: performanceId,
                    PerformanceAttributes: ["IMAX 70mm"],
      ...
      const day1 = parseRegalDatePayload(
        makeDatePayload("2026-07-28T00:00:00", 4000, "2026-07-28T14:00:00.000Z"),
        "1484",
        "2026-07-28",
        seen
      );
      const day2 = parseRegalDatePayload(
        makeDatePayload("2026-07-29T00:00:00", 4000, "2026-07-28T14:00:00.000Z"),
        "1484",
        "2026-07-29",
        seen
      );

      expect(day1.showtimes).toHaveLength(1);
      // Same PerformanceId, same startsAt as day1 — deduped once the seen map
      // is shared across both per-date calls.
      expect(day2.showtimes).toHaveLength(0);
      expect(day2.stats.dupSameStart).toBe(1);
    });

    it("without a shared seen map, per-date calls do NOT dedup across dates (documents pre-fix behavior)", () => {
      const makeDatePayload = (dateIso: string, performanceId: number, utcShowTime: string) => ({
        showDate: dateIso,
        shows: [
          {
      ...
      const day1 = parseRegalDatePayload(
        makeDatePayload("2026-07-28T00:00:00", 5000, "2026-07-28T14:00:00.000Z"),
        "1484",
        "2026-07-28"
      );
      const day2 = parseRegalDatePayload(
        makeDatePayload("2026-07-29T00:00:00", 5000, "2026-07-28T14:00:00.000Z"),
        "1484",
        "2026-07-29"
      );

      expect(day1.showtimes).toHaveLength(1);
      expect(day2.showtimes).toHaveLength(1);
    });
  });
  ...
  describe("parseRegalDatePayload", () => {
    // The per-date entry point probeHorizon's fetchDate calls: one payload in,
    // one payload's worth of showtimes/stats out.
    it("parses a single date's payload the same way parseRegalJson does", () => {
      const payload = {
        showDate: "2026-07-28T00:00:00",
        shows: [
          {
      ...
      const { showtimes, stats } = parseRegalDatePayload(payload, "1484");
    ...
    it("falls back to the query date when the payload carries no advertised date", () => {
      const payload = {
        shows: [
          {
            TheatreCode: "1484",
                ...
                Performances: [
                  {
                    PerformanceId: 3001,
                    PerformanceAttributes: ["IMAX 70mm"],
                    UtcShowTime: "2026-08-19T02:00:00.000Z",
                  },
                ],
              },
            ],
      ...
      const { showtimes } = parseRegalDatePayload(payload, "1484", "2026-08-18");

      expect(showtimes).toHaveLength(1);
      expect(showtimes[0].showDate).toBe("2026-08-18");
    });

    it("prefers the payload's own advertised date over the query date", () => {
      const payload = {
        shows: [
          {
            TheatreCode: "1484",
      ...
      const { showtimes } = parseRegalDatePayload(payload, "1484", "2026-07-27");

      expect(showtimes[0].showDate).toBe("2026-07-28");
    });
  });
});
